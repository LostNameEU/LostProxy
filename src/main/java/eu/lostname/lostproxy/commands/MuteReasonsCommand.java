...
public class MuteReasonsCommand extends Command implements TabExecutor {
    ...
    public void execute(CommandSender commandSender, String[] strings) {
        if (strings.length == 0 || strings.length == 5 || strings.length >= 7) {
            ...
            commandSender.sendMessage(new MessageBuilder("§8» §c/mutereasons list §8" + Prefix.DASH + " §7Liste dir alle Mutegruende auf").addClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/mutereasons list").build());
            commandSender.sendMessage(new MessageBuilder("§8» §c/mutereasons add <ID> <Name> <Zeit> <Zeiteinheit> <Permission> §8" + Prefix.DASH + " §7Liste dir alle Mutegruende auf").addClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/mutereasons add NAME ZEIT ZEITEINHEIT PERMISSION").build());
            commandSender.sendMessage(new MessageBuilder("§8» §c/mutereasons <ID> §8" + Prefix.DASH + " §7Zeige Informationen über einen Mutegrund an").addClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/mutereasons ID").build());
            commandSender.sendMessage(new MessageBuilder("§8» §c/mutereasons <ID> set <id,name,time,timeunit,permission> <Wert> §8" + Prefix.DASH + " §7Bearbeite einen Mutegrund").addClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/mutereasons ID set ").build());
            commandSender.sendMessage(new MessageBuilder("§8» §c/mutereasons <ID> delete §8" + Prefix.DASH + " §7Lösche einen Mutegrund").addClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/mutereasons ID set ").build());
            ...
            if ("list".equals(strings[0])) {
                ...
                try {
                    ...
                    if (iMuteReason != null) {
                        ...
                        commandSender.sendMessage(new MessageBuilder("§8» §7Name §8" + Prefix.DASH + " §c" + iMuteReason.getName()).build());
                        commandSender.sendMessage(new MessageBuilder("§8» §7ID §8" + Prefix.DASH + " §c" + iMuteReason.getId()).build());
                        commandSender.sendMessage(new MessageBuilder("§8» §7Mutezeit §8" + Prefix.DASH + " §c" + (iMuteReason.getTime() == -1 ? "permanent" : iMuteReason.getTime() + " " + ETimeUnit.getDisplayName(iMuteReason.getTime(), iMuteReason.getETimeUnit()))).build());
                        commandSender.sendMessage(new MessageBuilder("§8» §7Berechtigung §8" + Prefix.DASH + " §c" + iMuteReason.getPermission()).build());
                        commandSender.sendMessage(new MessageBuilder("§8§m--------------------§r").build());
