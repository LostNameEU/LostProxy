...
public class ReasonManager {
    ...
    public void loadBanReasons() {
        database.getMongoDatabase().getCollection(MongoCollection.BAN_REASONS).find().sort(Sorts.ascending("_id")).forEach(document -> this.registedBanReasons.add(gson.fromJson(document.toJson(), IBanReason.class)));
    }

    public void loadMuteReasons() {
        database.getMongoDatabase().getCollection(MongoCollection.MUTE_REASONS).find().sort(Sorts.ascending("_id")).forEach(document -> this.registedMuteReasons.add(gson.fromJson(document.toJson(), IMuteReason.class)));
    }
